  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      showAlert(
        'success',
        'Senha alterada!',
        'Sua senha foi atualizada com sucesso. Por favor, faça login novamente.'
      )
      onSuccess()
    } catch (error: any) {
    ...
    <BaseModal isOpen={isOpen} className="w-full max-w-lg p-0" hideCloseButton>
      <div className="bg-gradient-to-r from-yellow-400 to-yellow-500 px-8 py-10 text-center relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col items-center">
          ...
          <h2 className="font-title text-2xl font-black text-gray-900 uppercase tracking-tighter leading-none mb-2">
            Segurança da Conta
          </h2>
          <p className="font-body text-gray-800 text-sm font-bold opacity-80 uppercase tracking-widest">
      ...
      <div className="p-8 md:p-10 space-y-8 bg-gray-50/50">
        <div className="bg-yellow-50 border border-yellow-100 p-4 rounded-2xl flex gap-3 items-start">
          ...
          <p className="text-xs text-yellow-800 font-medium leading-relaxed">
            Este é seu primeiro acesso ou sua senha precisa ser atualizada. Por favor, escolha uma
            senha forte para continuar.
          </p>
        </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-2">
              {passwordRules.map((rule, idx) => (
                <div key={idx} className="flex items-center gap-2">
                  ...
                  <span
                    className={`text-[10px] font-bold ${rule.met ? 'text-gray-900' : 'text-gray-400'} transition-colors`}
                  >
                    {rule.label}
                  </span>
                </div>

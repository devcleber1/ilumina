    ...
    return (
      <main className={`flex-1 bg-gray-100 min-h-screen transition-all duration-300 ${!open ? 'pl-8' : ''}`}>
        <div className="flex w-full items-center justify-between px-6 py-4 bg-white shadow-sm sticky top-0 z-40">
          <div>
            <h1 className="font-title text-xl font-extrabold text-gray-900">Controle de Presença</h1>
            <p className="font-body text-xs text-gray-400">Selecione uma oficina para realizar a chamada</p>
          </div>
    ...
    <main className={`flex-1 bg-gray-100 min-h-screen transition-all duration-300 ${!open ? 'pl-8' : ''}`}>
      <div className="flex w-full items-center justify-between px-6 py-4 bg-white shadow-sm sticky top-0 z-40">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="font-title text-xl font-extrabold text-gray-900">{selectedWorkshop.nome_oficina}</h1>
            <p className="font-body text-xs text-gray-400">Chamada do dia {new Date(date + 'T12:00:00').toLocaleDateString('pt-BR')}</p>
          </div>
        </div>
      ...
      <div className="p-6 max-w-4xl mx-auto">
        {loadingStudents ? (
          ...
          <div className="bg-white rounded-3xl p-12 text-center border border-gray-100 shadow-sm">
            ...
            <p className="text-gray-400 text-sm mt-2">Esta oficina ainda não possui alunos matriculados.</p>
          </div>
        ) : (
                ...
                >
                    ...
                    <div className="absolute inset-0 bg-white/60 backdrop-blur-[1px] z-20 flex items-center justify-center p-4">
                      <div className="bg-white/90 border border-gray-100 px-4 py-2 rounded-2xl shadow-xl flex items-center gap-3 animate-in zoom-in-95 duration-300">
                        ...
                        <div className="flex flex-col">
                          <span className="text-[10px] font-black text-gray-900 uppercase tracking-tighter">Registro Bloqueado</span>
                          <span className="text-[9px] text-gray-500 font-medium">Limite de 2 edições atingido</span>
                        </div>
                      </div>
                    </div>
                  )}
                    ...
                    <div className="flex items-center gap-4">
                       <div className="h-12 w-12 rounded-2xl bg-gray-100 overflow-hidden border border-gray-200">
                          {student.foto_perfil_url ? (
                            <img src={`http://localhost:3001${student.foto_perfil_url}`} alt="" className="h-full w-full object-cover" />
                          ) : (
                       ...
                       <div>
                          <h4 className="font-title font-bold text-gray-900">{student.nome_completo}</h4>
                          <span className="text-[10px] text-gray-400 font-medium tracking-wider">MATRÍCULA: {student.numero_matricula}</span>
                       </div>
                    </div>
                  ...
                  {!record.presente && (
                    <div className="mt-4 pt-4 border-t border-red-50">
                       <label className="text-[10px] font-bold text-red-400 mb-1.5 block uppercase tracking-wider">Justificativa da Ausência</label>
                       <textarea 
                  ...
                  {record.total_edicoes !== undefined && record.total_edicoes > 0 && (
                     <div className="mt-3 flex items-center gap-1.5 text-[9px] text-gray-400 italic">
                        ...
                        <span>Registro atualizado {record.total_edicoes} {record.total_edicoes === 1 ? 'vez' : 'vezes'}</span>
                     </div>
                  )}
                </div>
              )
